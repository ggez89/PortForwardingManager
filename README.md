...
1. Open qBittorrent › Tools › Options.
    ...
    1. Set Web UI › Username and Password (optional, if Web UI authentication is enabled, configure these in `config.ini` in the installation directory).
...
If Web UI authentication is enabled in qBittorrent, specify your credentials in `config.ini` located in the application installation directory:
```ini
[qBittorrent]
Username = admin
...
```
This file is automatically generated with default values when `PortForwardingService` starts for the first time.

### Logging
By default, log messages are written to the text file `logs\PortForwardingService.log` in the installation directory.
